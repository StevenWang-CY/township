...
**RoundManager — `backend/simulation/round_manager.py`.** The core loop, one instance per town. It walks the scenario's `round_plan` and dispatches each declared phase in order: `seed` (full briefing → initial opinion), `converse` (in-town pairs chosen by a seeded, relationship-aware matching — declared ties and shared concerns pull people together, last rounds' partners are pushed apart — meeting where their routines put them at the round's clock, 3-exchange conversations), `news` (inject beats, collect reactions), `opinion` (reflect on the 10 most recent memories, re-form opinion), `decide` (mark agents decided — no LLM call). Every phase both mutates agent state (memories, opinions) and publishes events. A provider error marks the agent `ERROR` rather than minting a confident fake opinion.
