...
def _fence_pen(m: MapCanvas, x: int, y: int, w: int, h: int,
               gate: tuple[int, int] | None = None) -> None:
...
def compose(m: MapCanvas) -> None:
    ...
    trail |= {(x, y) for x in range(48, 55) for y in range(41, 48)
              if (x - 51) ** 2 + (y - 44) ** 2 <= 7}
    path(m, trail)
    ...
    for bx in (13, 22):
        ...
        m.stamp("buildings-top", R.BANNER_RED_A if bx == 13 else R.BANNER_RED_B,
                bx, 21)
    ...
    for dx in range(-3, 4):
        for dy in range(-3, 4):
            if abs(dx) + abs(dy) <= 3:
                m.set("ground-detail", 21 + dx, 38 + dy,
                      rng.choice(R.PATH_TAN.fill))
    ...
    # woods
    park_trees = [(53, 39, "tree_round_small"), (47, 45, "tree_round_small"),
                  (67, 33, "tree_light"), (71, 36, "tree_dark"),
                  (66, 41, "tree_light"), (64, 45, "tree_light"),
                  (73, 44, "tree_dark"), (69, 47, "tree_light"),
    ...
    for tx, ty, ts in [(15, 3, "tree_light"), (18, 5, "tree_dark"),
                       (20, 3, "tree_round_small")]:
        m.tree(tx, ty, stamp=ts)
    for tx, ty, ts in [(24, 12, "tree_light"), (27, 14, "tree_dark"),
                       (22, 14, "tree_round_small")]:
        m.tree(tx, ty, stamp=ts)
    for tx, ty, ts in [(33, 3, "tree_light"), (37, 4, "tree_dark"),
                       (40, 6, "tree_light")]:
        m.tree(tx, ty, stamp=ts)
    for tx, ty, ts in [(59, 6, "tree_dark"), (63, 4, "tree_light"),
                       (66, 2, "tree_dark"), (71, 4, "tree_light"),
                       (74, 7, "tree_dark")]:
        m.tree(tx, ty, stamp=ts)
    for tx, ty, ts in [(1, 31, "tree_light"), (4, 34, "tree_dark"),
                       (2, 38, "tree_light")]:
        m.tree(tx, ty, stamp=ts)
    m.stamp("deco-below", R.FERN, 5, 37)
    m.stamp("deco-below", R.ROCK_SMALL, 1, 35)
    for tx, ty, ts in [(28, 47, "tree_light"), (33, 48, "tree_dark"),
                       (41, 48, "tree_light")]:
        m.tree(tx, ty, stamp=ts)
    ...
    for lm in m.landmarks.values():
        m.anchor("label", lm.x + lm.w / 2 - 0.5, lm.y + lm.h / 2 - 1,
                 name=lm.name, text=lm.name)
