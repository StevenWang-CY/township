...
def tudor_shop(m: MapCanvas, x: int, y: int, w: int,
               door_dx: int | None = None, sign: int | None = None,
               awning: bool = False, door: str = "red") -> None:
    ...
    m.stamp("buildings-base", facade_wall("cream", w, rows=[4, 5]),
            x, y + 5)
    ...
    if awning:
        ...
        if w >= 5:
            ...
            sx = x + dd
        m.stamp("buildings-base", R.SIGNS_WALL[sign % len(R.SIGNS_WALL)],
                sx, y + 4)
    m.collide(x, y, w, 7)
...
def compose(m: MapCanvas) -> None:
    ...
    # -- Bay Street Station
    grand(m, 55, 30, 9, 7, facade="stone_large", roof="stone", door="metal",
          windows=True)
    m.pave(54, 37, 12, 2)
