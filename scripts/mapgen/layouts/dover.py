...
def compose(m: MapCanvas) -> None:
    ...
    storefront(m, 35, 16, 4, 6, facade="brick", roof="stone", sign=4,
               window=False)                                      # taqueria
    ...
    m.stamp("buildings-top", facade_wall("stone_small", 2, rows=[0, 1, 1, 2]),
            64, 11)
    m.anchor("smoke", 64.5, 10)
    m.stamp("buildings-top", R.METAL_GRATE, 59, 14)
    storefront(m, 67, 16, 4, 6, facade="stone_small", roof="stone",
               window=False)
    ...
    for y in (43, 45):
        for x in range(0, 75):
            m.set("ground-detail", x, y,
                  M.mg("rail_x") if x in crossing else M.mg("rail_h"))
    ...
    for i, fx in enumerate(range(59, 62)):
        m.stamp("deco-below", fy["rail_h_a" if i % 2 == 0 else "rail_h_b"],
                fx, 11)
    ...
    north_forest = [(2, 5), (7, 4), (23, 5), (28, 4), (33, 5), (26, 6),
                    (40, 5), (36, 3)]
