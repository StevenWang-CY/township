...
def compose(m: MapCanvas) -> None:
    ...
    storefront(m, 29, 30, 5, 5, facade="cream", roof="deck_light",
               awning=True)
    ...
    storefront(m, 4, 30, 4, 6, facade="brick", roof="deck_dark",
               window=False)
    storefront(m, 8, 30, 4, 6, facade="cream", roof="deck_light",
               window=False)
    storefront(m, 12, 30, 4, 6, facade="brick", roof="deck_dark",
               window=False)
    ...
    m.blob("ground-detail", {(55, 12), (56, 12), (55, 13), (56, 13)},
           R.GRASS_LIGHT, holes=False)
    m.blob("ground-detail", {(61, 16), (62, 16), (61, 17), (62, 17)},
           R.GRASS_LIGHT, holes=False)
    ...
    m.collide(52, 11, 3, 6)
    mid = TileStamp("ruin_mid", tuple(
        (br[r][5], br[r][6], br[r][7]) for r in (3, 4, 5)))
    m.stamp("buildings-base", mid, 55, 14)
    m.collide(55, 14, 3, 3)
    stub = TileStamp("ruin_stub", (
        (0, br[4][5], 0),
    ...
    stack = TileStamp("mill_stack", ((br[0][2], br[0][3]),) + tuple(
        (br[2 + r % 4][5], br[2 + (r + 2) % 4][5]) for r in range(6)))
    ...
    for i, x in enumerate(range(7, 9)):
        m.stamp("deco-below", f["rail_h_a" if i % 2 == 0 else "rail_h_b"],
                x, 40)
    ...
    # ================= SE orchard =================
    for ox, oy, st in ((55, 41, "tree_fruit_a"), (59, 44, "tree_fruit_b"),
                       (63, 40, "tree_fruit_a"), (67, 44, "tree_fruit_c"),
                       (71, 41, "tree_fruit_a")):
    ...
    for x, y in ((2, 1), (6, 2), (12, 1), (17, 2), (22, 1), (27, 2),
                 (41, 2), (47, 1), (52, 2), (58, 1), (64, 2), (70, 1),
                 (73, 2)):
        m.tree(x, y, stamp=rng.choice(("tree_dark", "tree_dark",
                                       "tree_light")))
    ...
    for lm in m.landmarks.values():
        lx, ly = label_pos.get(
            lm.name, (lm.x + lm.w / 2 - 0.5, lm.y + lm.h / 2 - 1))
