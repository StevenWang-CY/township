...
``frontend/public/assets/maps/<town>.tmj`` with the layer contract TownScene
binds to:
...
A town may ship a hand-tuned layout module ``scripts/mapgen/layouts/<town>.py``
exporting ``compose(m: MapCanvas)``; without one, a generic interpreter
composes every landmark by type.
...
T = 16
MAP_W, MAP_H = 75, 50
LAYER_NAMES = ("ground", "ground-detail", "deco-below",
               "buildings-base", "buildings-top")
...
def pad_stamp(stamp: TileStamp, w: int, h: int, name: str = "") -> TileStamp:
    ...
    cols = [0] + _cycle(range(1, W - 1), max(0, w - 2)) + \
        ([W - 1] if w > 1 else [])
    rws = [0] + _cycle(range(1, H - 1), max(0, h - 2)) + \
        ([H - 1] if h > 1 else [])
    return TileStamp(name or f"{stamp.name}_{w}x{h}",
                     tuple(tuple(rows[r][c] for c in cols) for r in rws))
...
def facade_wall(name: str, w: int, rows: list[int] | None = None,
                with_arch: bool = False) -> TileStamp:
    ...
    return TileStamp(f"wall_{name}_{w}",
                     tuple(tuple(stamp.gids[r][c] for c in cols)
                           for r in row_idx))
...
def awning_strip(w: int) -> TileStamp:
    ...
    return TileStamp(f"awning_{w}",
                     tuple(tuple(rows[r][c] for c in cols) for r in (0, 1)))
...
class MapCanvas:
    ...
    def __init__(self, town_id: str, town: dict, seed: int = 7) -> None:
        ...
        for lm in town.get("landmarks", []):
            self.landmarks[lm["name"]] = Landmark(
                lm["name"], lm.get("type", "building"),
                round(lm["x"] / T), round(lm["y"] / T),
                max(1, round(lm["width"] / T)), max(1, round(lm["height"] / T)),
                lm)
    ...
    def fill(self, layer: str, x: int, y: int, w: int, h: int,
             choices) -> None:
    ...
    def building_stamp(self, s: TileStamp, x: int, y: int,
                       top_rows: int) -> None:
    ...
    def blob(self, layer: str, cells: set[tuple[int, int]], b: Blob,
             holes: bool = True, fringe: bool = False) -> None:
        ...
        if fringe:
            ...
            for (x, y) in cells:
                for dx, dy, opts in ((0, -1, b.fringe_n), (0, 1, b.fringe_s),
                                     (-1, 0, b.fringe_w), (1, 0, b.fringe_e)):
                    p = (x + dx, y + dy)
                    if not opts or p in cells or p in self.reserved \
                            or not self.inb(*p):
                        continue
                    if self.get(layer, *p) == 0:
                        self.set(layer, *p, rng.choice(opts))

    def blob_rect(self, layer: str, x: int, y: int, w: int, h: int,
                  b: Blob, holes: bool = True, fringe: bool = False) -> None:
        self.blob(layer, {(xx, yy) for xx in range(x, x + w)
                          for yy in range(y, y + h) if self.inb(xx, yy)},
                  b, holes, fringe)
    ...
    def anchor(self, kind: str, x: float, y: float, name: str = "",
               **props) -> None:
        """Point anchor. (x, y) tile coords of the sprite's bottom-center
        (fractions allowed)."""
        p = {"kind": kind, **props}
        self.anchors.append({"name": name, "x": (x + 0.5) * T,
                             "y": (y + 1.0) * T, "props": p})

    def tree(self, x: int, y: int, stamp: str = "tree_light",
    ...
    def paint_roads(self, sidewalks: bool = True, dashes: bool = True,
                    crosswalks: bool = True) -> None:
        ...
        if sidewalks:
            ...
            for (x, y) in road:
                for dx in (-1, 0, 1):
                    for dy in (-1, 0, 1):
                        p = (x + dx, y + dy)
                        if p not in road and p not in self.reserved \
                                and self.inb(*p):
                            ring.add(p)
        ...
        if sidewalks:
            ...
            corner_gids = {M.mg("asp_nw"), M.mg("asp_ne"),
                           M.mg("asp_sw"), M.mg("asp_se")}
            for (x, y) in road:
                if self.get("ground-detail", x, y) in corner_gids:
                    self.set("ground", x, y, self.rng.choice(M.SIDEWALK.fill))
            swk_corner = {M.mg("swk_nw"): (-1, -1), M.mg("swk_ne"): (1, -1),
                          M.mg("swk_sw"): (-1, 1), M.mg("swk_se"): (1, 1)}
            for (x, y) in ring:
        ...
        if dashes:
            for seg in self.road_segs:
                ...
                for a in range(seg.a0, seg.a1 + 1):
                    ...
                    if any(jx0 - 1 <= cell[0] <= jx1 + 1
                           and jy0 - 1 <= cell[1] <= jy1 + 1
    ...
    def _junctions(self) -> list[tuple[int, int, int, int]]:
        ...
        for h in hs:
            for v in vs:
                if h.a0 <= v.c + v.width - 1 and v.c <= h.a1 \
                        and v.a0 <= h.c + h.width - 1 and h.c <= v.a1:
                    out.append((v.c, h.c, v.c + v.width - 1,
                                h.c + h.width - 1))
        return out
    ...
    def flowers(self, x: int, y: int, n: int = 5, spread: int = 3) -> None:
        for _ in range(n):
            ...
            if self.inb(fx, fy) and self.get("deco-below", fx, fy) == 0 \
                    and self.get("ground-detail", fx, fy) == 0 \
                    and (fx, fy) not in self.reserved:
                self.set("deco-below", fx, fy,
                         self.rng.choice(R.FLOWERS_WHITE))
...
def storefront(m: MapCanvas, x: int, y: int, w: int, h: int,
               facade: str = "brick", roof: str = "deck_dark",
               door_dx: int | None = None, sign: int | None = None,
               awning: bool = False, window: bool = True) -> None:
    ...
    if awning:
        ...
        m.stamp("buildings-top", R.SIGNS_WALL[sign % len(R.SIGNS_WALL)],
                sx, y + roof_h)
    m.collide(x, y, w, h)


def grand(m: MapCanvas, x: int, y: int, w: int, h: int,
          facade: str = "stone_gray", roof: str = "stone",
          banners: bool = False, windows: bool = False,
          door: str = "wood") -> None:
...
def cottage(m: MapCanvas, x: int, y: int, w: int = 6, h: int = 8,
            roof: str = "deck_light") -> None:
...
def apron(m: MapCanvas, x: int, y: int, w: int = 2, h: int = 1,
          material: str = "sidewalk") -> None:
...
def interpret_landmarks(m: MapCanvas) -> None:
    ...
    for lm in lms:
        ...
        if lm.type == "water":
            ...
            for _i in range(3):
                m.tree(x + 1 + m.rng.randrange(max(1, lm.w - 4)),
                       y + 1 + m.rng.randrange(max(1, lm.h - 3)),
                       stamp=m.rng.choice(("tree_light", "tree_dark")))
            m.flowers(x + w // 2, y + h - 2, n=6)
        elif lm.type == "church":
            grand(m, x, y + max(0, h - 8), min(w, 8), min(h, 8),
                  facade="stone_gray")
        elif lm.type == "civic":
            grand(m, x, y, min(w, 9), min(h, 8), facade="stone_large",
                  banners=True)
            ...
            storefront(m, x, y, min(w, 9), max(5, min(h, 7)),
                       facade=m.rng.choice(("brick", "cream")),
                       sign=m.rng.randrange(5), awning=m.rng.random() < 0.5)
    m.paint_roads()
...
def to_tmj(m: MapCanvas) -> dict:
    ...
    for name in LAYER_NAMES:
        ...
        layers.append({
            ...
            "opacity": 1, "x": 0, "y": 0, "width": m.w, "height": m.h,
            "data": data,
        })
        lid += 1
    oid = 1
    ...
    for (x, y, w, h) in m.collision:
        objs.append({"id": oid, "name": "", "type": "", "rotation": 0,
                     "visible": True, "x": round(x, 1), "y": round(y, 1),
                     "width": round(w, 1), "height": round(h, 1)})
        oid += 1
    layers.append({"id": lid, "name": "collision", "type": "objectgroup",
                   "visible": True, "opacity": 1, "x": 0, "y": 0,
                   "draworder": "topdown", "objects": objs})
    lid += 1
    aobjs = []
    for a in m.anchors:
        props = [{"name": k, "type": "string", "value": str(v)}
                 for k, v in sorted(a["props"].items())]
        aobjs.append({"id": oid, "name": a["name"], "type": "", "rotation": 0,
                      "point": True, "visible": True,
                      "x": round(a["x"], 1), "y": round(a["y"], 1),
                      "width": 0, "height": 0, "properties": props})
        oid += 1
    layers.append({"id": lid, "name": "anchors", "type": "objectgroup",
                   "visible": True, "opacity": 1, "x": 0, "y": 0,
                   "draworder": "topdown", "objects": aobjs})
    lid += 1
    ...
    return {
        ...
        "width": m.w, "height": m.h, "tilewidth": T, "tileheight": T,
        "nextlayerid": lid, "nextobjectid": oid,
        "tilesets": [
            {"firstgid": 1, "name": "rpg-tileset",
             "image": "../tilesets/rpg-tileset.png",
             "imagewidth": 1600, "imageheight": 1600,
             "tilewidth": T, "tileheight": T,
             "columns": 100, "tilecount": 10000, "margin": 0, "spacing": 0},
            M.tileset_json_entry(),
        ],
        "layers": layers,
    }
...
def build_town(scenario: str, town_id: str, out_dir: Path = MAPS_DIR) -> Path:
    town_path = REPO_ROOT / "scenarios" / scenario / "towns" / f"{town_id}.json"
    town = json.loads(town_path.read_text())
    m = MapCanvas(town_id, town, seed=abs(hash(town_id)) % 10000)
    m.base_grass()
    try:
        mod = importlib.import_module(f"mapgen.layouts.{town_id}")
...
def main() -> None:
    ...
    ap.add_argument("--preview", action="store_true",
                    help="also render the preview png(s)")
    args = ap.parse_args()
    towns_dir = REPO_ROOT / "scenarios" / args.scenario / "towns"
    towns = ([p.stem for p in sorted(towns_dir.glob("*.json"))]
             if args.all else [args.town])
    if not towns or towns == [None]:
        ap.error("pass --town <id> or --all")
    for t in towns:
        build_town(args.scenario, t)
        if args.preview:
            from mapgen import render_preview
            render_preview.render(t, scenario=args.scenario, labels=True)
